...
| **Identity** | `catalog.identity_user`, `catalog.identity_credential`, `catalog.identity_invitation`, `catalog.user_tenant_membership`, `catalog.authentication_event` | Authentication, credentials, MFA state, Tenant Membership, invitations, the platform authentication trail | `ICurrentUser`, `ITenantMembership`, `IAdministratorInvitation`, `IAuthenticationEventSink`, `AuroraClaimTypes`, sign-in commands | Store authorization. It answers *who*, never *what may they do*. It is the only place a `tid` claim is minted (ADR-0029 §4) |
| **Access** | `access` (tenant DB) | Roles, permissions, Role Assignments scoped to tenant **and** company, permission catalogue, the per-tenant seeder | `IPermissionEvaluator` (**not** `IAuthorizationService` — that name collides with `Microsoft.AspNetCore.Authorization.IAuthorizationService`, ADR-0029 §5), `Permission` constants, `IAccessSeeder`, role admin commands | Be bypassable, or return a third outcome. Every application-service command declares a permission (ADR-0010) and every path that cannot grant, denies (ADR-0029 §5) |
