...
|---|---|---|---|---|
| **Tenancy** | `catalog.*` (catalog DB) + `platform.tenant_identity` (tenant DB) | Tenant registry, routing, clusters, provisioning saga, migration runner, `TenantScope`, `ITenantDbContextFactory` | `TenantAccess`/`TenantScope`, `TenantAccessReason`, `SchemaVersion`/`CoreSchemaVersion`, `InstalledPackages`, `TenantRoutingViolationException`, `ITenantDbContextFactory<T>`, `ITenantScopeFactory`, `ITenantMigrationContextFactory<T>`, `IModuleSchemaMigrator`, tenant lifecycle commands (ADR-0027). **Not public** — see the note below the table | Contain a single line of business logic. It is the only module allowed to build a connection string |
| **Identity** | `catalog.identity_user`, `catalog.identity_credential` (readable only by `aurora_identity`), `catalog.identity_invitation`, `catalog.user_tenant_membership`, `catalog.authentication_event` | Authentication, credentials, MFA state, Tenant Membership, invitations, the platform authentication trail | `ICurrentUser`, `ITenantMembership`/`IdentitySnapshot`, `IPlatformUserProvisioning`, `IInvitationDelivery`, `IAuthenticationEventSink`, `AuroraClaimTypes`, sign-in commands | Store authorization. It answers *who*, never *what may they do*. It is the only place a `tid` claim is minted, and an invitation is never a capability over an established identity (ADR-0029 §4, A1.1 B-2) |
| **Access** | `access` (tenant DB) | Roles, permissions, Role Assignments scoped to tenant **and** company, permission catalogue, the per-tenant seeder | `AccessSubject` (constructing one from a principal *is* the tenancy comparison, ADR-0029 A1.2 H-1), `IPermissionEvaluator` (**not** `IAuthorizationService` — that name collides with `Microsoft.AspNetCore.Authorization.IAuthorizationService`), `Permission` constants, `IAccessSeeder`, role admin commands | Be bypassable, or return a third outcome. Every application-service command declares a permission (ADR-0010); every path that cannot grant, denies; and no path may hold both halves of the tenancy proof without comparing them |
...
**Tenancy: what is deliberately *not* in the public contract** (ADR-0034 §6, ADR-0035 §3.2, ADR-0027 §2). `ITenantConnectionResolver` and `TenantConnection` are `internal` to `Aurora.Platform.Tenancy` because `TenantConnection.ConnectionString` is a live credential and a type in a `.Contracts` assembly is nameable by every module, host and job. `TenantDatabaseHandle`, `ITenantAdminConnectionFactory`, `ITenantSchemaMigrator` and `TenantIdentityStamp` are internal to the same assembly because they are the DDL path: modules may reference `Aurora.Platform.*.Contracts` only (fitness rules L1–L5), so those four are unreachable by project reference, and fitness rule T6 is the second line rather than the only one. `ITenantMigrationContextFactory<T>` stays in `.Contracts` — a module's `IModuleSchemaMigrator` uses it — and it therefore **does not name `TenantDatabaseHandle`** in its signature (ADR-0035 §3.3).
