...
| 4 | **Changeability** | This system must be maintainable for a decade by people who never met us. Module boundaries, dependency direction and the Country Package contract are the mechanism. | Architecture fitness tests for layer and module rules (ADR-0029); ADRs; module contract assemblies. |
