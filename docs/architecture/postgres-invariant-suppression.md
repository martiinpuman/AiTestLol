...
|---|---|---|---|---|
| **S1** | `pg_trigger.tgenabled` | `D` never fires · `R` fires **only** when `session_replication_role = 'replica'`, so never for an ordinary write · `O` does not fire under `'replica'`, so it is weaker than `A` | `ALTER TABLE … DISABLE TRIGGER` (`D`) · `… ENABLE REPLICA TRIGGER` (`R`) · `… ENABLE TRIGGER` (`O`, a reduction from `A`) · `… ENABLE ALWAYS TRIGGER` (`A`, the top — the only one that cannot reduce) | `executed` for `R` (PR #9's review, PostgreSQL 17.11: the `TRUNCATE` landed while the predicate `tgenabled <> 'D'` reported the guard live) |
| **S2** | `pg_rewrite.ev_enabled` | The same four-value scheme as S1 | `ALTER TABLE … {DISABLE / ENABLE / ENABLE REPLICA / ENABLE ALWAYS} RULE` | `asserted` — the scheme is the same as S1's; the equivalence has not been executed here |
...
| **S8** | `pg_constraint.condeferrable` / `condeferred`, plus `SET CONSTRAINTS` | A deferrable constraint trigger checked at commit rather than per statement, and deferrable past the work | `CREATE CONSTRAINT TRIGGER … DEFERRABLE INITIALLY DEFERRED` · `ALTER TABLE … ALTER CONSTRAINT …` · `SET CONSTRAINTS … DEFERRED` | `asserted`. **Scope:** only *constraint* triggers and foreign keys are deferrable. B-19's guards are plain `BEFORE` triggers and are not, so this row does not reach them today — and would the day a guard is written as a constraint trigger |
...
| **S10** | `pg_event_trigger.evtenabled` | The same four-value scheme as S1, for event triggers | `ALTER EVENT TRIGGER … {DISABLE / ENABLE / ENABLE REPLICA / ENABLE ALWAYS}` | `asserted`. Nothing on this project uses event triggers today |
