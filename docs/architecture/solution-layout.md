...
| 4 | `BEFORE UPDATE OR DELETE … FOR EACH ROW` guard raising `42501`, on the parent **and created on every partition, under the parent guard's name plus `_local`**, enabled **`ALWAYS`**, and asserted with **`tgparentid = 0`** (**ADR-0028 Amendment 2**) | Probed **as `aurora_migrator`** — the owner whom privileges do not restrain — **inside a transaction that is rolled back**, requiring SQLSTATE `42501` and not merely *an* exception: through the parent, directly against a partition, and against a partition created *after* the guard exists. **Why its own name:** the clone owns the parent's name and cannot be dropped — executed, `CREATE TRIGGER` with the same name fails `already exists` in **both** orders and `DROP TRIGGER` on the clone is refused. **Why `tgparentid = 0`:** a clone and a partition-local guard are identical in `tgname`, `tgtype` and `tgenabled`. **Why not the clone at all:** `DETACH PARTITION` **deletes cloned triggers** — executed on the clone-fallback shape, `DETACH` → `DELETE 2` → `ATTACH` took a three-row trail to one while the locator printed `leaf 2 / guarded 2 / parents 1` and the effect probe printed `probed 4 / refused 4 / silent (none)` throughout. With the partition-local guard the same `DELETE` returns `42501`. It also stops sanctioned retention work, which detaches partitions, from unprotecting one for the duration. **The probe seeds its own row**, inside the same rolled-back transaction, in **every** partition including empty ones and `DEFAULT` (whose seed needs an `occurred_at` no month covers): a `FOR EACH ROW` trigger does not fire when no row matches, so an unseeded probe reported `probed: 2, refused: 0, silent: h.ev_09, h.ev_default` over two **correct** guards — executed. That false red is dangerous in its own way: the repair an implementer reaches for is to accept "no exception, zero rows" as a pass, and that acceptance can never fail. Seeded: `probed: 2, refused: 2, silent: (none)`, rollback leaving `rows_after 0`. **Why rolled back:** the run that finds a broken guard is the run in which the `DELETE` succeeds. **The destructive body for a row guard is `RETURN COALESCE(NEW, OLD)`, not `RETURN NULL`:** executed, `RETURN NULL` on a row trigger *cancels* the statement — `DELETE 0`, `UPDATE 0`, every row intact — so a demonstration built on it proves no data loss, though the probe still records the relation as silent |
| 5 | `BEFORE TRUNCATE … FOR EACH STATEMENT` guard on the parent **and on every partition, the `DEFAULT` partition included, created on the partition by the same migration step that creates or attaches it**, named `…_local`, enabled **`ALWAYS`**, asserted with **`tgparentid = 0`** — **ADR-0028 Amendment 2 (2026-09-12)**, replacing this row's earlier *“attached by the partition-creation job to every partition it creates”*. **What is asserted is refusal, not presence:** as `aurora_migrator`, inside a rolled-back transaction, execute `TRUNCATE` against **every** relation in `pg_partition_tree('audit.audit_event')` and require SQLSTATE **`42501`** from each; the probe reports how many refused and **names any that were silent**. **The binding check (v8):** for every guard trigger found here, join `pg_trigger.tgfoid` to `pg_proc` and assert `md5(prosrc)` of **the function the trigger actually calls** equals a constant **held in the test**, not in a table, **and** that `tgqual IS NULL`; report how many triggers were examined and fail on zero. The metadata enumeration stays as the **locator** that says *which* relation and *why*, and is not the guarantee | The old form guarded the parent and nothing else, and hung the rest on a job that does not exist (`FOLLOWUP-031`) which would never have created the `DEFAULT` partition anyway. Truncate triggers are **not** cloned, by `PARTITION OF` or by `ATTACH PARTITION`. **Executed on 17.11, guard on the parent only:** `TRUNCATE audit.audit_event` refused, then `TRUNCATE audit.audit_event_default` and `TRUNCATE audit.audit_event_2026_09` each returned `TRUNCATE TABLE` and took a three-row trail to **zero**. **Working:** four truncate paths refused, `count(*)` unchanged; the effect probe prints `relations probed 3, refused 3, silent (none)`. **Goes red:** a partition created with a bare `CREATE TABLE … PARTITION OF` — `leaf 2 / guarded 2` → `leaf 3 / guarded 2` **and** the owner's `TRUNCATE` on it succeeds. **Six ways to implement this wrong and still be green, each executed:** (a) checking presence instead of effect — `CREATE OR REPLACE FUNCTION … RETURN NULL` is one statement, no table DDL, and leaves the locator's output **byte-identical** while the trail goes to zero; (b) probing as **`aurora_app`** — an **unguarded** partition returns `42501: permission denied for table` from the privilege check, so the probe passes with no trigger present, which is why it runs as the owner, whose privilege path cannot produce `42501`; (c) asserting *“an exception was raised”* rather than the SQLSTATE; (d) `tgenabled IN ('O','A')` or `<> 'D'` — `ENABLE REPLICA TRIGGER` leaves `'R'`, and `session_replication_role = 'replica'` suppresses `'O'` with `tgenabled` unchanged; `ENABLE ALWAYS` on the **parent** reaches cloned triggers only, so a partition's own guard stays `'O'`; (e) omitting `tgparentid = 0` — the clone satisfies the predicate, which counts 1 where the clause counts 0; (f) a **session-keyed body** — with `if current_setting('aurora.maintenance', true) = 'on' then return coalesce(new, old); end if;` in front of the `RAISE`, the effect probe printed `refused with 42501: 2, silent: (none)`, the locator `always_guards 2`, and one `SET` later `DELETE 2` and `TRUNCATE TABLE` took the table to zero — **only the binding check catches this**, and storing its constant in the database instead of the test hands it to the same role; (g) pinning the guard function **by name** instead of by `tgfoid` — `CREATE OR REPLACE TRIGGER … EXECUTE FUNCTION <look-alike>` re-points the trigger while the named function's text, and its hash, stay exactly as recorded (executed: `calls v.refuse_lookalike whose body is a9b1dfd3` against a pinned `8b36e8b5…`); (h) not asserting `tgqual IS NULL` — PostgreSQL 17.11 accepts a `WHEN` clause on a row trigger **and** on a statement `TRUNCATE` trigger, verified, and `WHEN (coalesce(current_setting('aurora.maint', true),'off') <> 'on')` left the locator green, the effect probe returning `42501` on both paths and the by-name fingerprint at its constant — then one `SET` gave `DELETE 3` and `TRUNCATE TABLE` and an empty trail. Omitting the rollback also goes red — and empties the trail while doing it |
| 6 | Every privilege and trigger probe reports **how many** relations, roles and ACL entries it compared and fails on zero; the effect probe of criteria 4 and 5 reports **relations probed / refused with `42501` / silent**, naming the silent ones; the metadata enumeration prints `leaf_partitions`, `guarded_leaves` and `row_guarded_leaves` and fails when `leaf_partitions < 2`; the binding check prints **how many triggers it examined** and fails on zero | `CLAUDE.md` self-check #2. A privilege test that found no relations must not print PASS, and a guard enumeration over an empty partition tree must not print a clean `0 of 0`: the floor is the initial month partition plus `DEFAULT`. The floor is on the **population**, not on the guarded count — a floor asserted on `guarded_leaves` passes when both are zero. `row_guarded_leaves` is counted with `tgparentid = 0`, or it counts clones; the binding check that examines zero triggers has measured nothing and must not print PASS. **All three checks are needed and ADR-0028 Amendment 2 carries the executed six-row matrix that proves it:** the clone fallback is caught by the locator alone (the effect probe is green, because a clone refuses while it is attached); a session-keyed body, a `WHEN` clause and a re-pointed trigger are caught by the **binding check alone** (locator and effect probe green in all three, each one `SET` or one statement from an empty table); a neutered body by the effect probe and the binding check; an unguarded partition by the locator and the effect probe. Three checks, three different blind spots |
