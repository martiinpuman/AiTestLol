...
Status: accepted **v7** · Author: architect · Date: 2026-09-11
...
ADR-0029 §7, A1.2 H-8, A1.3 M-4, **A2.1**; ADR-0028 §2, §3. *Split out of B-18.1, which was over the size rule.*

1. A second additive migration on `CatalogDbContext` adds `catalog.authentication_event`, **monthly RANGE-partitioned** on `occurred_at` with a `DEFAULT` partition (the shape of `audit.audit_event`, ADR-0028 §3), append-only by all three mechanisms of ADR-0028 §2 applied **per table** — the `catalog` schema also holds mutable tables — probed as `aurora_app` **and** as the owner with `has_table_privilege` plus an executed `UPDATE` and `DELETE` that both fail.
