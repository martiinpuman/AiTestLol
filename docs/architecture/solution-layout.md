...
| 4 | Both guards exist on the parent **and on every partition**, created on the partition by the same migration step that creates or attaches it, named `…_local`, enabled **`ALWAYS`** — **ADR-0028 Amendment 2**. **What is asserted is refusal, not presence:** as `aurora_migrator`, inside a transaction that is **rolled back**, execute `TRUNCATE`, `UPDATE` and `DELETE` against **every** relation in `pg_partition_tree('audit.audit_event')` and require SQLSTATE **`42501`** from each, not merely *an* exception | **The probe seeds its own rows, and seeds them all before it attempts anything.** A `FOR EACH ROW` trigger does not fire when no row matches: unseeded, `DELETE` on an **empty but correctly guarded** partition returns `DELETE 0` with no exception and the probe reported `probed: 2, refused: 0, silent: h.ev_09, h.ev_default` over two **correct** guards (executed). That false red is dangerous because the repair an implementer reaches for — accepting "no exception, zero rows" as a pass — can never fail. **Seeding must also precede the whole loop, not be interleaved with it:** a draft that seeded each leaf as it reached it reported `silent: w.ev:delete` on a correct system, because the parent was probed before any leaf had a row (executed). Seeded, seeds first: `probed 9, refused with 42501: 9, silent (none)`, rollback leaving the table untouched. The `DEFAULT` partition's seed needs an `occurred_at` no month partition covers. **Why rolled back:** the run that finds a broken guard is the run in which the statement succeeds. **Why as the owner:** connected as `aurora_app` an **unguarded** partition returns `42501: permission denied for table` from the privilege check, so the probe passes with no trigger present (executed); the owner holds `TRUNCATE`, so its privilege path cannot produce `42501`. **The destructive body for a row guard is `RETURN COALESCE(NEW, OLD)`** — `RETURN NULL` merely cancels the statement (`DELETE 0`, executed) |
| 5 | **The binding is a whole-row comparison, not a list of predicates.** Pin the **entire `pg_trigger` row** of every guard the migration creates and the **entire `pg_proc` row** of the function each calls; compare the live catalog against that set **as a set** — missing, unexpected and changed are all differences — **column by column, every column**. Not compared, and this is the whole list: `oid` (the row's own identity); `tgrelid` and `tgname` (the comparison key); and every **oid-valued** column, compared by **resolved name** instead of its numeric value (`tgfoid` → `regprocedure`, `tgparentid` → `relation.trigger` or `none`, `tgconstrrelid`/`tgconstrindid` → `regclass` or `none`, `tgconstraint` → the constraint name or `none`). The population is **every non-internal trigger on every relation in `pg_partition_tree`, parent and clones included** | **This replaces four rounds of predicates because four rounds each found the next unread column** — the body, `tgparentid`, `tgqual` and `tgfoid`, `tgtype`. `pg_trigger` has **19** columns in 17.11 and the predicates read six. Executed, the comparison catches all four **and two nobody had reached**: `tgattr` (`BEFORE UPDATE OF seq` → `tgattr is [2], pinned []`) and `proconfig` (`ALTER FUNCTION … SET search_path`), with no predicate written for either. **Round 4's blocker:** `CREATE OR REPLACE TRIGGER … BEFORE UPDATE` takes a guard from `tgtype 27` to `19`, dropping `DELETE`, while it stays partition-local, `'A'`, unconditioned and calling the shipped body — every predicate green, then `DETACH` → `DELETE 2` → `ATTACH` took three rows to one. The comparison reports `tgtype is 19, pinned 27`. **Population, executed:** re-pointing the **parent's** row trigger at a look-alike left the predicate form at `examined: 7` with no finding, because its population was `tgparentid = 0` on leaves; the comparison reports three findings, `ev.g_row`, `ev_09.g_row`, `ev_default.g_row`. **Why default-compare is load-bearing:** a column PostgreSQL adds next is compared automatically and shows up as a difference, so it cannot be a gap nobody thought of. The one thing that could still slip is a new **oid-valued** column needing a resolver, so the check enumerates the catalog's columns at runtime and **refuses to run** on one it cannot resolve — executed with a resolver removed: `UNRESOLVED oid-valued column(s) in pg_trigger: tgconstraint — classify them before this check can claim completeness`. **Ways to implement this wrong and still be green, each executed:** scoping the population to `tgparentid = 0` or to leaves (round 3's error); comparing a hand-written list of columns instead of every column (rounds 1–4); pinning the guard function **by name** rather than through `tgfoid` — `CREATE OR REPLACE TRIGGER … EXECUTE FUNCTION <look-alike>` leaves the named function's text and hash exactly as recorded; and comparing `prosrc` alone rather than the whole `pg_proc` row, which misses `proconfig` and `prosecdef` |
| 6 | Every privilege and trigger probe reports **how many** relations, roles and ACL entries it compared and fails on zero; the effect probe reports **relations probed / refused with `42501` / silent**, naming the silent ones; the whole-row comparison reports **how many triggers and how many column comparisons** it performed; the population assertion prints `leaf_partitions`, `guarded_leaves` and the `DEFAULT` count | `CLAUDE.md` self-check #2. On the reference shape the comparison prints `triggers: 8, column comparisons: 368` — 8 guards × (17 trigger columns + 29 function columns) — and a comparison that examined **zero** triggers has measured nothing and must not print PASS. The floor is on the **population**, not on the guarded count: a floor asserted on `guarded_leaves` passes when both are zero. **All three mechanisms are needed and ADR-0028 Amendment 2 carries the executed thirteen-row matrix that proves it:** the comparison is red in twelve of thirteen breaks; the two it cannot see are a relation carrying no trigger (criterion 1 and the leaf floor) and a body that is wrong but unchanged since the migration (criterion 4's probe) |
