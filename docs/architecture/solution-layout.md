...
**Changes in v5** (2026-09-11, answering three review escalations): §6.4 records the acceptance criteria that change because **ADR-0028 Amendment 1** replaced §2 of that ADR, and the new rows that follow from it and from **ADR-0030** and **ADR-0031**. §6 and §6.1 are unchanged except where §6.4 says it overrides them.
...
**One limitation that needs a row of its own, not a sentence:** the hash chain does not detect truncation of the **tail** — removing the most recent rows leaves an internally consistent chain. ADR-0018's daily job writing the chain head to `catalog.operator_audit_event` is the answer, and nothing schedules it. Carry it as a follow-up (the project-manager assigns the id), depending on **§6.4 item 5**, which is the row that creates `catalog.operator_audit_event`.
