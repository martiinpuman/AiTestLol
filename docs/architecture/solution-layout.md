...
1. A second additive migration on `CatalogDbContext` adds `catalog.authentication_event`, **monthly RANGE-partitioned** on `occurred_at` with a `DEFAULT` partition (the shape of `audit.audit_event`, ADR-0028 §3). **Append-only follows ADR-0028 Amendment 2 and §6.4 item 1 criteria 1 and 4–6 in full — not the withdrawn ADR-0028 §2, and not the earlier form of this row.** The clause is **cited, not restated**: this row twice carried a partial version of it — a row guard “on the partitioned parent” and a truncate guard “attached to every partition the partition job creates” — and a partial enumeration of a clause reads as complete, which is how the guard that the original attack walked past survived three flaggings. `catalog.authentication_event` is a partitioned append-only table, so **every** part of the clause binds it: both guards created on each partition under the `_local` name, enabled `ALWAYS`, asserted with `tgparentid = 0`; the effect probe as `aurora_migrator` in a rolled-back transaction over every relation in `pg_partition_tree`, **seeding a row first**; the binding check on `tgfoid` and `tgqual`; the `DEFAULT` partition asserted to **exist** and be empty; the whole-row comparison of `pg_trigger` and `pg_proc` against what the migration created. **Criterion 1 is in that list deliberately:** its `DEFAULT`-partition *existence* check is what closes the concurrent-detach escalation, and a pointer that names only 4–6 drops it. If item 1 criteria 1 or 4–6 change, this row changes with them and needs no separate edit. What is specific to this table, and all this row states itself: (a) `aurora_app` holds **exactly** `SELECT, INSERT` on the **parent**, compared by `aclexplode(pg_class.relacl)` plus `pg_attribute.attacl` for `aurora_app` **and** `PUBLIC` against `CatalogSchemaAllowlist.AppRolePrivileges`, whose entry names its writer (B-18.11's limiter and B-18.5/B-18.6/B-18.7's refusal paths), then connected **as `aurora_app`** an `INSERT` that **succeeds** and `UPDATE`/`DELETE` that return `42501`; (b) **each partition's allow-list entry is present and empty** — §6.4 item 5 criterion 3, because `catalog` takes no default-privileges policy and a partition there keeps a null ACL; (c) **`ALTER DEFAULT PRIVILEGES` is not used in schema `catalog`** — §6.4 item 5 criterion 5 asserts `pg_default_acl` holds zero rows for it, and this row must keep that assertion true. Every probe reports how many relations, roles and ACL entries it compared and fails on zero.
...
### 6.4 Corrections and new rows (v11, 2026-09-12)

These follow **ADR-0028 Amendment 1**, **ADR-0030** and **ADR-0031**, all written the same day in response to the B-04, B-05 security and B-12 reviews. **v11 (2026-09-12) is the sixth security review of PR #9, and it is one correction to v10's own evidence.** v10's row-axis union made a pinned-and-not-live key a `MISSING` finding, which flipped the *`DEFAULT` partition dropped* row of ADR-0028 Amendment 2's matrix from green to **RED** — that partition carries three pinned triggers — and the matrix had not been re-run against the fix. All fourteen rows were re-executed; the comparison is red in fourteen of fourteen, so **the matrix no longer justifies criterion 1**, which stands on its `DEFAULT`-existence check and on the no-trigger case instead. The sixth review also implemented part 3 from the ADR text alone and got `triggers: 8, column comparisons: 360, findings: (none)`, which was the fifth review's stated acceptance criterion for the clause. **v10 (2026-09-12) is the fifth security review of PR #9, and it did not reopen the shape — it closed the clause around it.** The whole-row comparison specified the **live** side and left the **pinned** side unconstrained, so a name-keyed comparator that skips a column the pin does not carry was a conforming implementation that reproduced round 3's blocker in full (`column comparisons: 352, findings: 0`, partition emptied with one `SET`); criterion 5 now compares the **union of both sides on both axes**. The oid-resolution rule and the self-incompleteness guard were scoped to `pg_trigger` while the criterion mandates the whole `pg_proc` row, which has **nine** oid-valued columns — so the check refused to run on a correct system and the `pg_proc` half had no completeness guard; the rule is now catalog-independent with **fourteen** resolvers. Criterion 6's reference count `368` was unreachable and is **360**, derived as 8 × (16 + 29) and printed from the captured runtime arity. Resolvers are schema-qualified by construction, because a bare `::regprocedure` cast renders the same correct trigger differently under two `search_path`s. And criteria 4–6 now say in their own text that they are **migration-time assertions, not runtime health checks**.  **v9 (2026-09-12) changes the *shape* of criterion 5, on the fourth review's recommendation, and it is the only change here that is not a repair.** Four reviews found four blockers and **each one was a `pg_trigger` column nothing was reading** — the guard's body, `tgparentid`, `tgqual` and `tgfoid`, and now `tgtype`. Each round added a predicate; the next found the next column. Criterion 5 is therefore no longer a list of predicates but a **whole-row comparison** against the rows the migration created, with a five-name variance list, and criterion 4 keeps the effect probe as the separate question it answers. Executed, the comparison catches all four rounds plus `tgattr` and `proconfig`, neither of which anyone had written a predicate for. Also in v9: criterion 4 gains the probe's **seed-first ordering**; the B-16.2 tripwire gains the **mechanism that makes it fire**, which the natural implementation does not have; and every pointer to the clause now names **criterion 1** alongside 4–6, because criterion 1 is where the `DEFAULT`-existence check that closes the concurrent-detach escalation lives. **v8** was the third security review's three blockers and two mediums, and it is the first version to change a row **outside** §6.4: §6.2's `B-18.9` row (the one that creates `catalog.authentication_event`, the table the original attack emptied) still specified the withdrawn guard wording after three flaggings, and now **cites** ADR-0028 Amendment 2's clause instead of restating part of it. Inside §6.4: criterion 1's health check must assert the `DEFAULT` partition **exists**, not only that it is empty; criterion 4's probe **seeds its own row**, because a row trigger does not fire on an empty partition; criterion 5 gains the **binding check** on `tgfoid` and `tgqual`, because a fingerprint pinned by function name is green while the trigger calls something else or fires conditionally; the B-16.2 tamper cases gain a **recomputing** one, because an unkeyed chain is repaired in one `UPDATE` loop and every case specified so far was the non-recomputing form; and item 5 criterion 4 cites the clause rather than enumerating part of it. **v6 and v7 (both 2026-09-12) changed items 1 and 5 only**, following **ADR-0028 Amendment 2** and the two security reviews of it. **v7** is the second review's four findings: the partition-local guards need their own **names** (the clone owns the parent's and cannot be dropped) and are identified by **`tgparentid = 0`**, without which the clause is a convention with no mechanism; the guard **body** is pinned by a fingerprint held in the test, because a session-keyed body passes both probes; `RETURN COALESCE(NEW, OLD)` is the destructive body for a row guard; and the chain detects removal in the **middle and at neither end**, so the closing limitation is prefix **and** tail. **v6** was: item 1's criteria 4, 5 and 6 (both guards reach every partition, `DEFAULT` included, created by the statement that creates the partition rather than by a job that does not exist, enabled `ALWAYS`, and **asserted by executing the forbidden statement in a rolled-back transaction rather than by checking that a trigger exists**), its criterion 2 and its closing limitation; and item 5's criteria 2, 3 and 4 (a partition's expected ACL is recorded as **empty**, which is the catalog's shape and not `audit`'s). Items 2, 3 and 4 are untouched. Where an item overrides §6, §6.1 or a `../BACKLOG.md` row it says so. The architect does not edit the backlog; the project-manager transcribes these. **Row numbers here follow `../BACKLOG.md`'s split of the architect's single B-16 row — B-16.1 schema and append-only enforcement, B-16.2 the writer and hash chain, B-16.3 the `[Auditable]` interceptor — not §6.1's two-row form above.**

#### 1. B-16.1 — the append-only acceptance criteria, re-specified

**Overrides** the append-only half of §6.1's B-16.1 row and of `../BACKLOG.md`'s B-16.1 row. The partitioning half of both is unchanged. Reason: ADR-0028 §2 named a statement that does nothing, and the criterion built on it could not fail. ADR-0028 Amendment 1 carries the executed evidence; this is the row form. **Criteria 4, 5 and 6 were re-specified again on 2026-09-12 by ADR-0028 Amendment 2**, which carries the executed evidence in turn. The chain of corrections is worth reading before implementing: the original form guarded the parent and left every partition truncatable by the owner; Amendment 2's first draft then checked that a guard *existed* rather than that it *refused*, which the first PR #9 review broke with one `CREATE OR REPLACE FUNCTION`; and its second draft specified guards that **could not be created as worded**, which the second review broke by showing that the fallback an implementer is forced into reopens the hole with every published number unchanged.

| # | Criterion | What makes it a real check |
|---|---|---|
| 1 | Schema `audit`; `audit.audit_event` RANGE-partitioned monthly on `occurred_at`; a `DEFAULT` partition; every primary key and unique constraint includes `occurred_at`; the next two months pre-created by the platform job; a health check asserts **exactly one `DEFAULT` partition exists** and that it is empty, reporting both numbers | Partitioning unchanged from §6.1. **The health check's existence half is new (v8) and is not pedantry:** dropping the `DEFAULT` partition re-enables `DETACH PARTITION CONCURRENTLY`, which parks a partition outside `pg_partition_tree` and therefore outside criteria 4–6 (ADR-0028 Amendment 2). Executed on the shape a developer naturally writes — an aggregate over the partitions whose bound is `DEFAULT` — the check prints `default partition holds 0 rows` **whether the `DEFAULT` partition is present and empty or absent entirely**, because a sum over no rows is zero. The corrected form fails with `expected exactly 1 DEFAULT partition, found 0`. An emptiness check that cannot tell *empty* from *absent* is self-check #2 in another hat, and here *absent* is the dangerous state |
...
| 4 | **Criteria 4, 5 and 6 are migration-time assertions, not runtime health checks:** they run as `aurora_migrator` in the integration test that follows a migration of a freshly created database, are on no request path, and do **not** belong in the per-tenant health-check family of ADR-0007 §9.4 — putting one there would need a standing DDL-role credential in the runtime and an `ACCESS EXCLUSIVE` lock on every audit partition, the opposite of what ADR-0027 arranges (`FOLLOWUP-033` carries the live-system question). Both guards exist on the parent **and on every partition**, created on the partition by the same migration step that creates or attaches it, named `…_local`, enabled **`ALWAYS`** — **ADR-0028 Amendment 2**. **What is asserted is refusal, not presence:** as `aurora_migrator`, inside a transaction that is **rolled back**, execute `TRUNCATE`, `UPDATE` and `DELETE` against **every** relation in `pg_partition_tree('audit.audit_event')` and require SQLSTATE **`42501`** from each, not merely *an* exception | **The probe seeds its own rows, and seeds them all before it attempts anything.** A `FOR EACH ROW` trigger does not fire when no row matches: unseeded, `DELETE` on an **empty but correctly guarded** partition returns `DELETE 0` with no exception and the probe reported `probed: 2, refused: 0, silent: h.ev_09, h.ev_default` over two **correct** guards (executed). That false red is dangerous because the repair an implementer reaches for — accepting "no exception, zero rows" as a pass — can never fail. **Seeding must also precede the whole loop, not be interleaved with it:** a draft that seeded each leaf as it reached it reported `silent: w.ev:delete` on a correct system, because the parent was probed before any leaf had a row (executed). Seeded, seeds first: `probed 9, refused with 42501: 9, silent (none)`, rollback leaving the table untouched. The `DEFAULT` partition's seed needs an `occurred_at` no month partition covers. **Why rolled back:** the run that finds a broken guard is the run in which the statement succeeds. **Why as the owner:** connected as `aurora_app` an **unguarded** partition returns `42501: permission denied for table` from the privilege check, so the probe passes with no trigger present (executed); the owner holds `TRUNCATE`, so its privilege path cannot produce `42501`. **The destructive body for a row guard is `RETURN COALESCE(NEW, OLD)`** — `RETURN NULL` merely cancels the statement (`DELETE 0`, executed) |
| 5 | **The binding is a whole-row comparison, not a list of predicates, and both sides of it are closed.** Pin the **entire `pg_trigger` row** of every guard the migration creates and the **entire `pg_proc` row** of the function each calls; compare the live catalog against that set **as a set** — missing, unexpected and changed are all differences — **column by column, every column**, over the **union of the two sides on both axes**: a key pinned and not live is `MISSING`, a key live and not pinned is `UNEXPECTED`, a **live column the pin does not carry is `NO PINNED VALUE`** and a pinned column the catalog does not return is `PINNED BUT NOT IN CATALOG`. No branch may `continue` on a lookup miss. Not compared, per catalog: in `pg_trigger` `oid`, `tgrelid` and `tgname` (**three of nineteen, leaving sixteen**); in `pg_proc` `oid` (**one of thirty, leaving twenty-nine**). Every **oid-valued** column of **either** catalog — type `oid`, `oidvector`, `oid[]` or any `reg*` alias, decided from `pg_attribute.atttypid` at runtime — is compared by **resolved name**: **fourteen resolvers on 17.11, five in `pg_trigger` and nine in `pg_proc`**, enumerated in ADR-0028 Amendment 2 part 3, **every one schema-qualified by construction and never a bare `::regclass`/`::regprocedure`/`::regtype` cast**, with both the pinning and the comparing session running `search_path = pg_catalog`. The population is **every non-internal trigger on every relation in `pg_partition_tree`, parent and clones included** | **This replaces four rounds of predicates because four rounds each found the next unread column** — the body, `tgparentid`, `tgqual` and `tgfoid`, `tgtype`. `pg_trigger` has **19** columns in 17.11 and the predicates read six. Executed, the comparison catches all four **and two nobody had reached**: `tgattr` (`BEFORE UPDATE OF <column>` → `tgattr is '3', pinned ''`) and `proconfig` (`ALTER FUNCTION … SET search_path`), with no predicate written for either. **Round 5's blocker, and why the union rule is not decoration:** with the pin one column short, a name-keyed comparator that skips the miss printed `triggers: 8, column comparisons: 352, findings: 0` while a `WHEN (current_setting('aurora.maint', true) IS DISTINCT FROM 'on')` clause sat on the parent's `g_row` and on `ev_09.g_row_local` and `SET aurora.maint = 'on'; DELETE FROM a.ev_09;` emptied the partition — executed. Under the union rule the same state prints `triggers: 8, column comparisons: 360, findings: 8`, one `NO PINNED VALUE` per row. **Round 5's second blocker:** the resolver set and the refusal guard scoped to `pg_trigger` alone make the check **refuse to run on a correct system**, because `pg_proc` has nine oid-valued columns — and leave the half carrying `prosrc`, `prosecdef` and `proconfig` with no completeness guard at all. **Population, executed:** re-pointing the **parent's** row trigger at a look-alike left the predicate form at `examined: 7` with no finding; the comparison turns three trigger rows red — `ev.g_row`, `ev_09.g_row`, `ev_default.g_row` — for `findings: 9`, since `proname` and `prosrc` differ too. **Why default-compare is load-bearing:** a column PostgreSQL adds next is compared automatically, so the only thing that can still slip is a new **oid-valued** column needing a resolver — so the check enumerates **each** catalog's columns at runtime and **refuses to run** on one it cannot resolve, naming the catalog: `UNRESOLVED oid-valued column(s) in pg_proc: proallargtypes — classify them before this check can claim completeness`. **Ways to implement this wrong and still be green, each executed:** skipping a column the pin does not carry; scoping the population to `tgparentid = 0` or to leaves (round 3's error); scoping the oid rule or the refusal guard to `pg_trigger` (round 5's); comparing a hand-written list of columns instead of every column (rounds 1–4); pinning the guard function **by name** rather than through `tgfoid`; comparing `prosrc` alone rather than the whole `pg_proc` row, which misses `proconfig` and `prosecdef`; and resolving an oid through a bare cast, whose text depends on `search_path` — same database, same correct trigger, `refuse()` under one `search_path` and `a.refuse()` under another, executed |
| 6 | Every privilege and trigger probe reports **how many** relations, roles and ACL entries it compared and fails on zero; the effect probe reports **relations probed / refused with `42501` / silent**, naming the silent ones; the whole-row comparison reports **how many triggers and how many column comparisons** it performed; the population assertion prints `leaf_partitions`, `guarded_leaves` and the `DEFAULT` count | `CLAUDE.md` self-check #2. **Every number here is derived at runtime, not asserted.** The comparison first prints the arity it captured from the catalog enumeration — `captured: trigger columns 16, function columns 29` — and then `triggers: 8, column comparisons: 360, findings: (none)` on the reference shape, 8 guards × (16 + 29) = **360**. `column comparisons` is the sum over the keys present on **both** sides of that key's column union, so a live population of 7 gives 315 and an `UNEXPECTED` ninth key still gives 360 — the number moves with what was measured. **This value was published as `368` through four revisions and no correct implementation reaches it:** nineteen `pg_trigger` columns minus three variance is sixteen, not seventeen, and the wrong reference destroyed the only signal that separated *compared everything* from *compared what I pinned*. A run printing `triggers: 0` or `column comparisons: 0` has measured nothing and must fail, which is a different outcome from `findings: 0` over a non-zero count. The floor is on the **population**, not on the guarded count: a floor asserted on `guarded_leaves` passes when both are zero. **All three mechanisms are needed, and ADR-0028 Amendment 2 carries the executed fourteen-row matrix:** the comparison is red in **fourteen of fourteen** breaks, including the `DEFAULT` partition dropped, which the matrix published as green until it was re-run against the row-axis union (`triggers: 5, column comparisons: 225, findings: 3`, three `MISSING`). **So the matrix is not what justifies criteria 1 and 4** — what the comparison structurally cannot see appears in no row of it: (a) a relation carrying **no** trigger at all, which on this shape cannot be reached by dropping a partition, because the survivors' pinned keys go `MISSING`, but requires **the parent's row guard to be absent so nothing clones** — criterion 1's `DEFAULT`-existence check and the leaf floor are what stand there, and criterion 1 is independently non-redundant because dropping the `DEFAULT` partition re-enables `DETACH PARTITION CONCURRENTLY`; and (b) a body that is wrong but **unchanged since the migration**, which criterion 4's probe covers in its **unconditional** form only — the session-conditioned form shipped by the migration is past every criterion here and is stated as a residual in ADR-0028 Amendment 2 |
...
**Three additions to B-16.2** (the writer and hash chain). Its tamper test gains (a) disable the guard as the owner, edit a row, re-enable, and assert `VerifyAsync` reports the break; (b) drop a **middle** partition and assert the same; and — **new in v8, and the important one** — (c) drop a middle partition **and recompute the suffix in one `UPDATE` loop**, then assert `VerifyAsync` returns **clean**.

(c) is a test that pins a **limitation**, not a guarantee, and it is required because (a) and (b) are both of the **non-recomputing** form: they pass forever without ever exercising the attack. Executed — `BROKEN at seq 7` before the recompute, `VERIFIES CLEAN` after it, on both the link-only and the anchored verifier, six rows. The chain is unkeyed `SHA256` over a canonical form ADR-0028 §5 publishes, so recomputation needs nothing the attacker does not already have. Writing (c) as an assertion that verification **succeeds** pins the limitation — but on its own it has **no firing mechanism**, and the natural implementation gives it none: a recompute that calls the production hashing code becomes keyed along with the chain, so the tampered chain still verifies and (c) stays green forever.

**The mechanism, which must be built with it:** the recompute is implemented **in the test, from ADR-0028 §5's published construction, sharing no code with the writer** — which is also the only honest model of the attacker, who has the ADR and not our types — and (c) is preceded by one assertion: **the independently computed hash of every untampered row equals its stored hash**, reported with the row count. Executed: unkeyed, `published construction reproduces every stored hash (rows checked: 5)`; with the writer changed to prepend a key and nothing else touched, `TRIPWIRE FIRED at seq 6: the published construction no longer reproduces stored hashes (rows checked: 6)`. It fires on keying, on a canonical-form change and on an algorithm change alike, and the count stops it passing over an empty table. **The tripwire asserts over the whole fixture, every untampered row, not a sample** — a tripwire that checks the first row still reports a count and still looks like a count. Independently reproduced by the fifth review on nine rows over three monthly partitions, both directions: `published construction reproduces every stored hash (rows checked: 9)`, then, with the writer changed to prepend a key and nothing else touched, `TRIPWIRE FIRED at seq 10: the published construction no longer reproduces stored hashes (rows checked: 10)`. **Why re-implementing a published specification in a test is affordable here and must not be generalised:** this assertion pins a **limitation**, and a limitation test that shares code with the implementation cannot distinguish *the limitation still holds* from *the implementation moved underneath me* — the two outcomes it exists to tell apart. The cost is bounded because ADR-0028 §5 publishes the canonical form as a **stable contract** the project has independently promised not to change silently, and because divergence is loud and immediate rather than slow rot. Where the canonical form is an implementation detail rather than a published contract, the second copy is a tax with no theory behind it; this is not a licence to reimplement production code in tests generally. `CLAUDE.md` self-check #3: a tamper test that cannot fail against the tamper it is named for is not a check, and neither is a tripwire nobody can say how to trip.

**One limitation that needs a row of its own, not a sentence — and it is larger than this row previously said.** Executed against ADR-0028 §5's chain, nine rows over three monthly partitions: a row removed from inside a partition gives `BROKEN at seq 6`; a **middle** partition dropped gives `BROKEN at seq 7`; the **newest** partition dropped (tail) gives `VERIFIES CLEAN`; and the **oldest** partition dropped — **which is exactly what retention does** — also gives `VERIFIES CLEAN`. Partitions are monthly ranges on `occurred_at` and the chain is ordered `(occurred_at, sequence)`, so dropping the oldest partition is **prefix** removal, and the chain detects the middle and neither end. Anchoring the surviving first row to §5's 32 zero bytes catches the prefix case (executed: `BROKEN at seq 4`) and is **incompatible with retention**, which makes the anchor unsatisfiable after the first sanctioned drop.

**And against a role that can write, the chain detects none of it.** Executed: the middle-partition drop that gives `BROKEN at seq 7` returns to `VERIFIES CLEAN` after one `UPDATE` loop recomputing the suffix. So the chain detects accidental loss, storage corruption and an actor who does not recompute — not the DDL-capable role this paragraph is about. Whether the chain should be **keyed**, with an HMAC key held outside the tenant database, is a decision of its own and is **not** taken in ADR-0028 Amendment 2 or here; note that a key would not close the prefix case either, since prefix removal leaves every surviving link valid under any key.

Criterion 5 above closes the half of this that needed no DDL. What is left is a role that can `DROP`/`DISABLE` the guard, change what it calls or when it fires, replace its body, or detach and drop a partition, and **nothing detects that**. The cover — ADR-0018's daily job writing the chain head to `catalog.operator_audit_event`, **outside the tenant database**, depending on **§6.4 item 5** (carried as **`B-19`**) and **`FOLLOWUP-031`** — covers the **tail** and not the prefix: executed, tail removal moves `last_seq` 9 → 6 and changes `last_hash`, while prefix removal leaves both **unchanged** and moves only the row count, which retention moves too. Separating them needs retention to publish its expected effect on that count — **`FOLLOWUP-034`**'s second half, not this row's and not this document's. Carried in `../BACKLOG.md` as **`FOLLOWUP-026`**, whose scope is the tail.
...
| 4 | A `BEFORE UPDATE OR DELETE … FOR EACH ROW` guard raising `42501` on both tables, created in the same migration as the table and enabled **`ALWAYS`** (ADR-0028 Amendment 2) | Probed **as `aurora_migrator`** — the owner, whom privileges do not restrain — **inside a transaction that is rolled back**, requiring SQLSTATE `42501` and not merely *an* exception. Remove or disable the guard and the probe gets `UPDATE 1` instead of `42501`; replace the guard **function's body** with `CREATE OR REPLACE FUNCTION … RETURN COALESCE(NEW, OLD)` — the destructive body for a **row** guard, since `RETURN NULL` merely cancels the statement (`DELETE 0`, executed) — and every presence check still reports green while the `DELETE` succeeds. That is why the probe executes the statement, why it rolls back, and why item 1 criterion 5's **whole-row comparison** applies here too, against the `pg_trigger` and `pg_proc` rows this migration creates: a body keyed on a session setting passes the probe and is one `SET` away from `DELETE 2` (executed), and so do a `WHEN` clause, a re-pointed `tgfoid` and a `tgtype` that has quietly lost `DELETE`. These tables are unpartitioned, so the population is two triggers rather than eight, and the comparison is otherwise identical. Neither table is partitioned, so item 1's clone and truncate-trigger questions do not arise here; say that in the migration rather than leaving a reader to wonder why criterion 5 is absent. **When a partitioned append-only table does join `catalog` — `catalog.authentication_event` is one, created by `B-18.9`'s own additive migration, which is how the project-manager answered ADR-0029 A2.8's ownership question when it assigned `B-19` to this item — ADR-0028 Amendment 2's clause applies to it in full, and is **cited rather than restated here**. An earlier form of this cell listed part of it (`tgenabled = 'A'`) and omitted the rest (`tgparentid = 0`, the `_local` names, the row guard, the seed, the binding check); a partial enumeration of a clause reads as complete and is worse than a pointer. **The pointer is: item 1 criteria 1 and 4–6, every part** — criterion 1 included, because its `DEFAULT`-existence check is what closes the concurrent-detach escalation. This is not hypothetical: executed on 17.11 in a `catalog` schema with no default privileges, `TRUNCATE catalog.authentication_event_default` as `aurora_migrator` emptied the partition past the parent's guard — the third ADR-0029 security review's finding, reproduced |
