...
| 5 | `BEFORE TRUNCATE … FOR EACH STATEMENT` on the parent **and on every partition, the `DEFAULT` partition included, created by the same migration statement that creates or attaches the partition** — **ADR-0028 Amendment 2 (2026-09-12)**, which replaces this row's earlier *“attached by the partition-creation job to every partition it creates”*. The invariant to assert, and the only thing actually enforced: every relation in `pg_partition_tree('audit.audit_event')` carries a non-internal trigger with `tgtype & 32 <> 0` (TRUNCATE), `tgtype & 2 <> 0` (BEFORE) and **`tgenabled IN ('O','A')`** | The old form guarded the parent and nothing else, and hung the rest on a job that does not exist (`FOLLOWUP-031`) and that would never have created the `DEFAULT` partition anyway. Truncate triggers are **not** cloned, by `PARTITION OF` or by `ATTACH PARTITION`. **Executed on 17.11, guard on the parent only:** `TRUNCATE audit.audit_event` refused, then `TRUNCATE audit.audit_event_default` and `TRUNCATE audit.audit_event_2026_09` each returned `TRUNCATE TABLE` and took a three-row trail to **zero**. **Working:** as `aurora_migrator`, truncate the parent, a month partition, the `DEFAULT` partition, and both partitions in one statement — four `42501`s and an unchanged `count(*)`. **Failing, produced in the suite rather than asserted here:** create a partition with a bare `CREATE TABLE … PARTITION OF` that skips the helper — the enumeration goes from `leaf 2 / guarded 2` to `leaf 3 / guarded 2` **and** the owner's `TRUNCATE` on it succeeds. **`tgenabled <> 'D'` is the wrong predicate:** `ENABLE REPLICA TRIGGER` leaves `'R'`, which it counts as a guard while the `TRUNCATE` succeeds (executed, both predicates side by side) |
| 6 | Every privilege and trigger probe reports **how many** relations, roles and ACL entries it compared, and fails on zero — and criterion 5's guard enumeration prints `leaf_partitions` and `guarded_leaves` and fails when `leaf_partitions < 2` | `CLAUDE.md` self-check #2. A privilege test that found no relations must not print PASS, and a guard enumeration over an empty partition tree must not print a clean `0 of 0`: the floor is the initial month partition plus `DEFAULT` |
...
| 4 | A `BEFORE UPDATE OR DELETE … FOR EACH ROW` trigger raising `42501` on both tables, created in the same migration as the table | Probed **as `aurora_migrator`** — the owner, whom privileges do not restrain. Remove or disable the trigger and the probe gets `UPDATE 1` instead of `42501`. Neither table is partitioned, so item 1's clone and truncate-trigger questions do not arise here; say that in the migration rather than leaving a reader to wonder why criterion 5 is absent. **When a partitioned append-only table does join `catalog` — `catalog.authentication_event` is one, wherever ADR-0029 A2.8's ownership question puts it — ADR-0028 Amendment 2's clause applies to it unchanged:** the `BEFORE TRUNCATE` guard on the parent **and on every partition including `DEFAULT`**, created by the statement that creates or attaches the partition, with item 1 criterion 5's `pg_partition_tree` enumeration and its `tgenabled IN ('O','A')` predicate. This is not hypothetical: executed on 17.11 in a `catalog` schema with no default privileges, `TRUNCATE catalog.authentication_event_default` as `aurora_migrator` emptied the partition past the parent's guard — the third ADR-0029 security review's finding, reproduced |
