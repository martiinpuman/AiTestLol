...
**Schema Version** — Which edition of Aurora ERP's own data layout a Tenant's system is currently at. A release of the software may reshape how records are stored, and each tenant's system is brought up to date in its own turn, so at any moment some tenants are one step behind the software. The software always knows the newest edition it produces and the oldest it still understands; a tenant whose system is older than that is blocked and shown a maintenance page rather than served answers from a layout the software no longer reads correctly.
...
**Tenant Access Reason** — Why the system entered a Tenant's system on a given occasion: a person's request, a scheduled job, the delivery of a queued event, setting the tenant up, bringing their data layout up to date, or an operator helping the customer. Every entry names exactly one of these six, so the audit trail can say not only which tenant was worked in but why.

**Tenant Identity Stamp** — A mark written into each Tenant's own system at the moment it is created, naming the tenant it belongs to. Before the software trusts any connection to a tenant's system it reads the mark and checks that it names the tenant it meant to reach; a system carrying another tenant's mark, or no mark at all, is refused outright rather than queried. It is what turns a misdirected connection — a wrong address, a backup restored under the wrong name — into a refusal instead of a leak.
...
**Tenant Scope** — The proof, carried through every piece of work, that the system knows which Tenant it is acting for: the tenant's identity and Tenant Key, where its data may live, its Schema Version, its installed Country Packages and the Tenant Access Reason. Only the tenancy core can issue one; nothing else in the system can make one up, which is what makes "no work without a known tenant" a rule the software enforces rather than a habit.
