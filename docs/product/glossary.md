...
**Migration Category** — Which of three kinds a database change is, declared on the change itself and checked by the build against what the change actually does. An **Expand** only adds — a new table, a new optional column, a new index — so the previous version of the software keeps working against the result while the change rolls out to every customer's database. A **Contract** takes away what an earlier Expand made redundant — drops, renames, tightenings — and may only ship a release after that Expand, which it names. A **DataOnly** change moves or fills in data and touches no structure at all. A change that claims one category and does another is refused before it can reach a single tenant.
