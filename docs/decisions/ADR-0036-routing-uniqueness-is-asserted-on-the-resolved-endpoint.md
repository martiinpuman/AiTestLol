...
> - **Non-ASCII folding is collation-dependent, and the deployment does not pin the collation.** Executed on `postgres:17-alpine` 17.11: `lower('Ü' collate "C")` returns `'Ü'` unchanged, while `lower('Ü')` under the database's `en_US.utf8` default returns `'ü'`. **So `host = lower(host)` does not mean the same thing on two catalog databases created with different `lc_ctype`.** An earlier reviewer could not reproduce a "folds ASCII only" witness, and this is why: neither "ASCII only" nor "full Unicode" is true of the constraint — the behaviour is a property of the database it was created in.
>
>   **The witness has to be chosen carefully, and three wrong forms of it circulated before anyone ran it under both collations.** `pg.ÜBER.internal` is **not** a witness: it contains the ASCII letters `B`, `E`, `R`, which are refused for a reason that has nothing to do with collation, so it is refused either way. The witness is **`pg.Über.internal`** — non-ASCII upper case and nothing else. Executed: `host = lower(host)` is **satisfied** by it under `COLLATE "C"` (nothing folds, so it equals itself) and **violated** under the database default and under ICU `en-US-x-icu`. On a `C`-collated catalog the lower-case constraint **accepts** it.
>
>   **Is §3's grammar load-bearing for the collation problem?** `FOLLOWUP-061` asks exactly this and calls the grammar's coverage "fortunate rather than designed". **It is load-bearing, and it is not luck.** Executed across three collation providers on 17.11 — the database default (libc `en_US.utf8`), `COLLATE "C"`, and ICU `COLLATE "en-US-x-icu"` — the grammar accepts `pg.uber.internal` under all three and refuses `pg.Über.internal`, `pg.über.internal` and `pg.ÜBER.internal` under all three. PostgreSQL evaluates a regex bracket range by code point, so `[a-z0-9]` does not widen under a linguistic collation, and the grammar's verdict is the same on every catalog however it was created.
>
>   **The link that carries that, named: "PostgreSQL regex bracket ranges are code-point based", verified on 17.11 across three providers.** It is not a documented stability guarantee across major versions, so it is a link to re-check at a PostgreSQL upgrade rather than a property to rely on silently. And it makes `ck_database_cluster_host_lower_case` redundant on this column rather than load-bearing — measured, now, instead of assumed.
>
>   **The general fact survives and is not discharged by any of that.** `ck_tenant_host_lower_case` governs `catalog.tenant_host` with **no grammar beside it**, so it inherits the problem undiminished, and any future case-folding constraint does too. **The catalog's collation is an input to a constraint's meaning and must be pinned at `createdb` time.** That is carried by **[`FOLLOWUP-061`](../BACKLOG.md)** — *"The catalog database's `lc_ctype` is not pinned, so `host = lower(host)` means different things on different catalogs"* — which records the same runs and which this record answers on the one question it put back to the architect. **Let each check do one job**: the grammar decides the character set, the pinned collation decides what folding means, and `lower()` is then belt on this column and the only control on `tenant_host`.
...
| §3 `lower()`'s meaning | The database's collation | **Not pinned anywhere.** Neutralised for this column by the grammar — measured across libc, `C` and ICU, resting on regex ranges being code-point based, which is a link to re-check at a PostgreSQL major upgrade. Live and uncovered for `ck_tenant_host_lower_case`; carried by `FOLLOWUP-061` |
...
- **One sentence in §3 was true when written and false a day later, without anyone editing it.** It said no backlog row carried the collation requirement — correct at the time, and falsified by the orchestrator opening `FOLLOWUP-061` *because* this record pointed at the gap. **A negative existence claim about the repository is the one kind of claim that rots when somebody does the right thing**, and it rots in a document whose purpose is to be trusted about what is and is not carried. The rule earned: **name the artefact that carries a requirement, not the absence of one** — and where something genuinely is uncarried, pin the claim to a commit so its staleness is checkable rather than assumed. Added to the conventions note.
