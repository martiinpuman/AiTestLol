...
3. **Errors are RFC 9457 Problem Details**, always, including validation failures (`errors` extension member keyed by field path). `type` is a stable, documented URI per error class; `traceId` is always present (ADR-0016). A stack trace never crosses the boundary.
   > **Extended by [ADR-0038](ADR-0038-contract-carried-text-names-its-sink.md) §3 (2026-09-12).** An exception's **`Message`** never crosses the boundary either, for the same reason as the stack trace: it is written for whoever has to fix the failure. The mapper is an **allowlist** from exception type to problem body, composing `detail` from the exception's structured properties; anything not on it becomes a generic `500` plus `traceId`. `TenantRoutingViolationException`'s message names another tenant's id and a database name, correctly, and must never reach a client. **No mapper exists yet** — ADR-0038 §3.3.
