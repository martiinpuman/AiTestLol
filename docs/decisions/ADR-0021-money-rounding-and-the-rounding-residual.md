...
- **Negative, and the one that already cost us a defect: `decimal` is exact in base 10, which is not the same as exact, and it does not tell you when it stops.** Multiplication and division round **silently** past 28–29 significant digits — no exception, no flag. A weight produced by `a / b` carries 28 decimal places, so `minorUnits * weight` is rounded before anything downstream sees it, and an algorithm that then relies on the result being a whole number of minor units quietly returns parts that do not sum to their total. The first implementation of §6 did exactly this: 20.6% of randomised ratio splits did not sum to the total, at ordinary SMB invoice magnitudes, with no code path able to notice. **The rule that follows: any money algorithm that multiplies a magnitude by a caller-supplied factor and then depends on the result being integral must either do the arithmetic in integers (`System.Numerics.BigInteger` — BCL, no dependency, no licence question) or verify its own postcondition before returning, and preferably both.** A property test guards it only if its generator actually reaches the region where the law breaks; weights drawn from a list of short literals never multiply inexactly and can never fail. This applies wherever an amount meets a proportion — allocation (§6), tax apportionment (ADR-0023 §6), landed-cost distribution, payment application and FX translation (§7).
