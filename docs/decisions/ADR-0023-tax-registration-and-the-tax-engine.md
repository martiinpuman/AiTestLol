...
- **Negative: tax apportionment is the next place ADR-0021's silent-`decimal` consequence bites, and it bites with a factor the core does not control.** `CalculateAsync` multiplies a line basis by a rate, and on a per-document rounding basis it apportions one document-level tax amount back across lines — magnitude × caller-supplied factor, twice, where the factor is **Country Package data**. A package supplying a rate or an apportionment weight with many significant digits is not a hypothetical: rates arrive as data precisely so that no core release is needed to change them, and nothing stops a package expressing one as a ratio. So the ADR-0021 rule applies here in full: **apportionment does the arithmetic in integers (`BigInteger`) and verifies before returning that the parts sum exactly to the document tax amount**, and the property test's rate and weight generators must be genuinely arbitrary rather than a list of the rates one jurisdiction happens to use. A tax total whose lines do not add up is a wrong filed return, not a rounding artefact — and the §5 category and exemption structure means the discrepancy can land inside a single category where no document-level check would see it.
