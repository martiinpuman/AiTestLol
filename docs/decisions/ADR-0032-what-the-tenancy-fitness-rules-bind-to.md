...
4. Faults that must turn a fixture red: make T10's owning set a prefix match (a `…Sales.Evil` fixture stops being reported); make T11 read declared accessibility instead of effective accessibility (the T12 fixture — a `public` member on an `internal` type — starts being reported by T11, which the step-3 assertion catches in either phase); restrict T12 to a member-name key (the `newobj` fixture stops being reported).
