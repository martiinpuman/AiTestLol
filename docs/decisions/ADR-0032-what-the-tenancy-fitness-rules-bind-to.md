...
4. **Whether L6** ("no `public` type in a module's `Application` or `Infrastructure`") **subsumes part of T11** once it is implemented. It probably narrows T11's population; it does not replace it, because T11 also covers platform assemblies.
