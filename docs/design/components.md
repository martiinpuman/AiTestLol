...
| Disabled | 40% opacity, `text-disabled` label, no pointer cursor, not focusable | same pattern | same | Disabled buttons always carry a reason reachable on hover/focus (tooltip: "Requires Approver role," "Period is closed") — a disabled control with no explanation is a dead end, not a state. |
...
- **No permission** (records may exist but the user can't see them) — states that plainly, without exposing whether data exists, and does not offer a create action the user isn't authorized for.
...
| Read-only / single-company tenant | Still renders (never hidden), showing the one company name as a static, unmistakable label — so the visual guarantee ("you can always see which company you're in") holds even for the tenants who never need to switch. |
